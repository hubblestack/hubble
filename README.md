...
```
With the file_root configuration added:
1. Run `hubble hubble.sync` to pull the files into Hubble's cache
1. Run `hubble hubble.load`
1. Run `hubble hubble.audit`. The agent should now only run the network.ssh profile.
