# -*- encoding: utf-8 -*-
"""
HubbleStack Audit-to-Splunk returner

Deliver HubbleStack Audit result data into Splunk using the HTTP
      ...
      returner:
        splunk:
          - token: XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX
            ...
            index: hubble
            sourcetype_audit: hubble_audit_v2
...
def returner(ret):
    """
    Get audit data and post it to Splunk
    """
    data = ret['return']
    if not isinstance(data, dict):
        log.error('Data sent to splunk_audit_return was not formed as a dict:\n%s', data)
        return
    ...
    try:
        opts_list = get_splunk_options(sourcetype='hubble_audit_v2',
                                       _nick={'sourcetype_audit': 'sourcetype'})
        ...
        log.exception('Error occurred in splunk_audit_return')
    return
...
def event_return(event):
    ...
    for event_data in event:
        ...
        elif event_data['data']['fun'] != 'audit.run':
            continue  # not a call to audit.run, so not relevant
        else:
