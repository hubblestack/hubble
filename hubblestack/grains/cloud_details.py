    ...
    try:
        ...
        if token_request.status_code == requests.codes.ok:
            ...
            if response.status_code == requests.codes.ok:
                ...
                cloud_account_id = int(aws["cloud_account_id"])
                aws["cloud_account_id"] = f"{cloud_account_id:0>12}"
            else:
...
def _get_azure_details():
    ...
    try:
        ...
        if response.status_code == requests.codes.ok:
            ...
            raise ValueError(
                "Error while fetching Azure instance metadata. Got status code: %s " % (response.status_code)
            )
    except (requests.exceptions.RequestException, ValueError) as e:
        log.debug(e)
        # Not on an Azure box
    ...
    if azure:
        try:
            ...
            if response.status_code == requests.codes.ok:
                ...
                raise ValueError(
                    "Error while fetching metadata for Azure instance: %s. Got status code: %s"
                    % (azure["cloud_instance_id"], response.status_code)
                )
        except (requests.exceptions.RequestException, ValueError) as e:
            log.debug(e)
            azure_extra = None
...
def _get_gcp_details():
    ...
    try:
        ...
        if response.status_code == requests.codes.ok:
            ...
            raise ValueError(
                "Error while fetching GCP instance metadata. Got status code: %s" % (response.status_code)
            )
...
def _build_gcp_extra(metadata):
    ...
    for counter, value in enumerate(interface_list):
        ...
        grain_name_accessconfig_external_ips = "cloud_interface_{0}_" "accessconfigs_external_ips".format(counter)
