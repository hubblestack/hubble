    ...
    try:
        ...
        for line in pcictl_out.splitlines():
            for vendor in known_vendors:
                vendor_match = re.match(r"[0-9:]+ ({0}) (.+) \(VGA .+\)".format(vendor), line, re.IGNORECASE)
                if vendor_match:
...
def _virtual(osdata):
    ...
    if osdata["kernel"] in choices:
        ...
        if os.path.isfile("/proc/1/cgroup"):
            try:
                ...
                if ":/lxc/" in fhr_contents:
                    ...
                    if any(x in fhr_contents for x in (":/system.slice/docker", ":/docker/", ":/docker-ce/")):
        ...
        if kenv:
            product = __mods__["cmd.run"]("{0} smbios.system.product".format(kenv))
            maker = __mods__["cmd.run"]("{0} smbios.system.maker".format(kenv))
        ...
        if sysctl:
            ...
            jail = __mods__["cmd.run"]("{0} -n security.jail.jailed".format(sysctl))
        ...
        if sysctl:
            if "QEMU Virtual CPU" in __mods__["cmd.run"]("{0} -n machdep.cpu_brand".format(sysctl)):
                grains["virtual"] = "kvm"
            elif "invalid" not in __mods__["cmd.run"]("{0} -n machdep.xen.suspend".format(sysctl)):
                grains["virtual"] = "Xen PV DomU"
            elif "VMware" in __mods__["cmd.run"]("{0} -n machdep.dmi.system-vendor".format(sysctl)):
                grains["virtual"] = "VMware"
            # NetBSD has Xen dom0 support
            elif __mods__["cmd.run"]("{0} -n machdep.idle-mechanism".format(sysctl)) == "xen":
...
def _virtual_hv(osdata):
    ...
    try:
        version = {}
        for func in ("major", "minor", "extra"):
            with hubblestack.utils.files.fopen("/sys/hypervisor/version/{}".format(func), "r") as fhr:
                version[func] = hubblestack.utils.stringutils.to_unicode(fhr.read().strip())
...
def _clean_value(key, val):
    ...
    if val is None or not val or re.match("none", val, flags=re.IGNORECASE):
        ...
        if (
            ...
            or re.search(r"sernum|part[_-]?number|specified|filled|applicable", val, flags=re.IGNORECASE)
        ):
            return None
...
def _linux_bin_exists(binary):
    ...
    for search_cmd in ("which", "type -ap"):
        try:
            return __mods__["cmd.retcode"]("{0} {1}".format(search_cmd, binary)) == 0
        except hubblestack.exceptions.CommandExecutionError:
            pass

    try:
        return len(__mods__["cmd.run_all"]("whereis -b {0}".format(binary))["stdout"].split()) > 1
    except hubblestack.exceptions.CommandExecutionError:
        return False
            ...
            with hubblestack.utils.files.fopen(filename) as ifile:
                ...
                for line in ifile:
                    ...
                    if match:
                        ...
                        ret[match.group(1)] = re.sub(r'\\([$"\'\\`])', r"\1", match.group(2))
            break
        except (IOError, OSError):
            pass
...
def os_data():
    ...
    if hubblestack.utils.platform.is_proxy():
        ...
        if _linux_bin_exists("selinuxenabled"):
            ...
            grains["selinux"] = {}
            grains["selinux"]["enabled"] = __mods__["cmd.retcode"]("selinuxenabled") == 0
            if _linux_bin_exists("getenforce"):
                grains["selinux"]["enforced"] = __mods__["cmd.run"]("getenforce").strip()
        ...
        try:
            ...
            for key, value in iter(release.items()):
                key = key.lower()
                lsb_param = "lsb_{0}{1}".format("" if key.startswith("distrib_") else "distrib_", key)
        ...
        log.trace("Getting OS name, release, and codename from " "platform.linux_distribution()")
        (osname, osrelease, oscodename) = [
            x.strip('"').strip("'") for x in linux_distribution(supported_dists=_supported_dists)
        ]
    ...
    if grains.get("osrelease", ""):
        ...
        os_name = grains[
            "os" if grains.get("os") in ("Debian", "FreeBSD", "OpenBSD", "NetBSD", "Mac", "Raspbian") else "osfullname"
        ]
...
def fqdns():
    ...
    addresses = hubblestack.utils.network.ip_addrs(include_loopback=False, interface_data=_INTERFACES)
    addresses.extend(hubblestack.utils.network.ip_addrs6(include_loopback=False, interface_data=_INTERFACES))
...
def _hw_data(osdata):
    ...
    if osdata["kernel"] == "Linux" and os.path.exists("/sys/class/dmi/id"):
        ...
        hubblestack.utils.platform.is_smartos()
