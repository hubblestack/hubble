...
# all relating to the persistqueue
import persistqueue
...
def get_splunk_options(subspace=False, other_defaults=tuple()):
    defaults = other_defaults + (
        ...
        ( ('http_event_server_ssl','hec_ssl'), True),
        ('proxy', {}),
        ('timeout', 9.05),
        ('index_extracted_fields', []),
    )

    config_space = __salt__['config.get']('hubblestack:returner:splunk', {})
    if subspace:
        config_space.extend(
            __salt__['config.get']('hubblestack:{0}:returner:splunk'.format(subspace, {})))
...
class DiskQueue(persistqueue.SQLiteQueue):
    max_size = 10000

    def __init__(self, fname='/var/cache/hubble/pq.db'):
        self._last_rfq = 0
        try:
            super(DiskQueue, self).__init__(fname)
        except SQLiteDBError:
            # XXX: is this dangerous?
            if os.path.isdir(fname):
                shutil.rmtree(fname)
            # one retry
            super(HEC, self).__init__(fname)

    def put(self, item):
        # XXX: max_size is in items, not bytes
        # is this better in bytes? is the limit too high? too low?
        if self.size < self.max_size:
            super(DiskQueue, self).put(item)
            self._last_rfq = 0
        else:
            t = time.time()
            dt = t - self._last_rfq
            if dt > queue_overflow_msg_delay:
                log.error("refusing to queue event item: queue full")
                self._last_rfq = t
