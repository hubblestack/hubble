...
def raw_mod(opts, name, functions, mod='modules'):
    ...
    .. code-block:: python
        ...
        __opts__ = salt.config.minion_config('/etc/salt/minion')
        testmod = hubblestack.hubble_mods.loader.raw_mod(__opts__, 'test', None)
        testmod['test.ping']()
    '''
    loader = LazyLoader(
...
def states(opts, functions, utils, serializers, whitelist=None, proxy=None):
    ...
    .. code-block:: python
        ...
        __opts__ = salt.config.minion_config('/etc/salt/minion')
        statemods = hubblestack.hubble_mods.loader.states(__opts__, None, None)
    '''
    ret = LazyLoader(
...
def grain_funcs(opts, proxy=None):
    ...
    Returns the grain functions
      ...
      .. code-block:: python
          ...
          __opts__ = salt.config.minion_config('/etc/salt/minion')
          grainfuncs = hubblestack.hubble_mods.loader.grain_funcs(__opts__)
    '''
    return LazyLoader(
        _module_dirs(
            opts,
...
def clouds(opts):
    ...
    functions = LazyLoader(
        ...
        opts,
        tag='clouds',
        pack={'__utils__': hubblestack.hubble_mods.loader.utils(opts),
