...
def show(config_file=False):
    ...
    if config_file:
        ...
        try:
            with hubblestack.utils.files.fopen(config_file, "r") as f:
...
def persist(name, value, config="/etc/sysctl.conf"):
    ...
    with hubblestack.utils.files.fopen(config, "r") as ifile:
        for line in ifile:
            line = hubblestack.utils.stringutils.to_unicode(line).rstrip("\n")
    ...
    with hubblestack.utils.files.fopen(config, "w+") as ofile:
        nlines = [hubblestack.utils.stringutils.to_str(_l) + "\n" for _l in nlines]
