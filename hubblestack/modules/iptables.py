...
# by hubble in this state module and not on the actual state module function
...
_STATE_INTERNAL_KEYWORDS = STATE_REQUISITE_KEYWORDS.union(
...
def _iptables_cmd(family="ipv4"):
    ...
    """
    if family == "ipv6":
        return hubblestack.utils.path.which("ip6tables")
    else:
        return hubblestack.utils.path.which("iptables")
...
def _has_option(option, family="ipv4"):
    ...
    if option in __mods__["cmd.run"](cmd, output_loglevel="quiet"):
        return True
    return False
...
def _conf_save_filters():
    ...
    .. code-block:: python
        ...
        _conf_save_filters()
    """
    config = __mods__["config.option"]("iptables.save_filters", [])
    return config
...
def version(family="ipv4"):
    ...
    out = __mods__["cmd.run"](cmd).split()
    return out[1]
...
def check(table="filter", chain=None, rule=None, family="ipv4"):
    ...
    if _has_option("--check", family):
        ...
        out = __mods__["cmd.run"](cmd, output_loglevel="quiet")
    else:
        _chain_name = hex(uuid.getnode())

        # Create temporary table
        __mods__["cmd.run"]("{0} -t {1} -N {2}".format(ipt_cmd, table, _chain_name))
        __mods__["cmd.run"](
            "{0} -t {1} -A {2} {3}".format(ipt_cmd, table, _chain_name, rule)
        )

        out = __mods__["cmd.run"]("{0}-save".format(ipt_cmd))

        # Clean up temporary table
        __mods__["cmd.run"]("{0} -t {1} -F {2}".format(ipt_cmd, table, _chain_name))
        __mods__["cmd.run"]("{0} -t {1} -X {2}".format(ipt_cmd, table, _chain_name))
...
def check_chain(table="filter", chain=None, family="ipv4"):
    ...
    out = __mods__["cmd.run"](cmd).find(":{0} ".format(chain))
...
def _parse_conf(conf_file=None, in_mem=False, family="ipv4"):
    ...
    rules = ""
    if conf_file:
        with hubblestack.utils.files.fopen(conf_file, "r") as ifile:
            rules = ifile.read()
    elif in_mem:
        cmd = "{0}-save".format(_iptables_cmd(family))
        rules = __mods__["cmd.run"](cmd)
    else:
        raise HubbleException("A file was not found to parse")
    ...
    for line in rules.splitlines():
        ...
        if line.startswith("*"):
            ...
            args = hubblestack.utils.args.shlex_split(line)
            index = 0
