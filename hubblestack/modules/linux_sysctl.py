...
def show(config_file=False):
    ...
    if config_file:
        ...
        try:
            with hubblestack.utils.files.fopen(config_file) as fp_:
                for line in fp_:
                    line = hubblestack.utils.stringutils.to_str(line)
...
def persist(name, value, config=None):
    ...
    nlines = []
    try:
        with hubblestack.utils.files.fopen(config, "r") as _fh:
            ...
            config_data = hubblestack.utils.data.decode(_fh.readlines())
    except (IOError, OSError):
    ...
    if not edited:
        nlines.append("{0} = {1}\n".format(name, value))
    try:
        with hubblestack.utils.files.fopen(config, "wb") as _fh:
            _fh.writelines(hubblestack.utils.data.encode(nlines))
    except (IOError, OSError):
        msg = "Could not write to file: {0}"
        raise CommandExecutionError(msg.format(config))
