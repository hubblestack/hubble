# -*- coding: utf-8 -*-
"""
Hubble module to manage Unix mounts and the fstab file
"""
import logging
import os
import re
...
import hubblestack.utils.mount
...
def _list_mounts():
    ...
    if __grains__["os"] in ["MacOS", "Darwin"]:
        mounts = __mods__["cmd.run_stdout"]("mount")
    else:
        mounts = __mods__["cmd.run_stdout"]("mount -l")
...
def _active_mountinfo(ret):
    ...
    if "disk.blkid" not in __context__:
        __context__["disk.blkid"] = __mods__["disk.blkid"]()
    blkid_info = __context__["disk.blkid"]

    with hubblestack.utils.files.fopen(filename) as ifile:
        for line in ifile:
            comps = hubblestack.utils.stringutils.to_unicode(line).split()
...
def _active_mounts_aix(ret):
    ...
    """
    for line in __mods__["cmd.run_stdout"]("mount -p").split("\n"):
        comps = re.sub(r"\s+", " ", line).split()
        if comps:
...
def _active_mounts_solaris(ret):
    ...
    """
    for line in __mods__["cmd.run_stdout"]("mount -v").split("\n"):
        comps = re.sub(r"\s+", " ", line).split()
        ret[comps[2]] = {
            "device": comps[0],
            "fstype": comps[4],
...
def _active_mounts_openbsd(ret):
    ...
    for line in __mods__["cmd.run_stdout"]("mount -v").split("\n"):
        ...
        if len(parens) > 1:
            nod = __mods__["cmd.run_stdout"]("ls -l {0}".format(comps[0]))
...
def _active_mounts_darwin(ret):
    ...
    """
    for line in __mods__["cmd.run_stdout"]("mount").split("\n"):
        comps = re.sub(r"\s+", " ", line).split()
    ...
    for ind, opt in enumerate(opts):
        if opt.split("=")[0] in name_id_opts:
            ...
            if not re.match("[0-9]+$", _givenid):
                _info = __mods__[name_id_opts[_param]](_givenid)
...
class _fstab_entry(object):
    ...
    def __init__(self, **criteria):
        ...
        items = [
            key_value
            for key_value in criteria.items()
...
def fstab(config="/etc/fstab"):
    ...
    if not os.path.isfile(config):
        return ret
    with hubblestack.utils.files.fopen(config) as ifile:
        for line in ifile:
    ...
    if modified:
        try:
            with hubblestack.utils.files.fopen(config, "wb") as ofile:
                ofile.writelines(hubblestack.utils.data.encode(lines))
        except (IOError, OSError) as exc:
...
def set_fstab(
    ...
    **kwargs
    ...
    if ret != "present":  # ret in ['new', 'change']:
        if not hubblestack.utils.args.test_mode(test=test, **kwargs):
            try:
                with hubblestack.utils.files.fopen(config, "wb") as ofile:
...
def set_automaster(
    ...
    **kwargs
    ...
    if not os.path.isfile(config):
        __mods__["file.touch"](config)
        __mods__["file.append"](automaster_file, "/-\t\t\t{0}".format(config))
...
def mount(
    ...
    if util != "mount":
        ...
        if util == "guestfs":
            return __mods__["guestfs.mount"](name, root=device)
        elif util == "qemu_nbd":
            mnt = __mods__["qemu_nbd.init"](name, device)
            if not mnt:
                return False
            first = next(mnt.values())
    ...
    if not os.path.exists(name) and mkmnt:
        __mods__["file.mkdir"](name, user=user)
    ...
    out = __mods__["cmd.run_all"](cmd, runas=user, python_shell=False)
    if out["retcode"]:
        return out["stderr"]
    return True
...
def umount(name, device=None, user=None, util="mount"):
    ...
    """
    if util == "guestfs":
        __mods__["guestfs.umount"](name, disk=device)
        return
    elif util == "qemu_nbd":
        # This functionality used to live in img.umount_image
        if "qemu_nbd.clear" in __mods__:
            if "img.mnt_{0}".format(name) in __context__:
                __mods__["qemu_nbd.clear"](__context__["img.mnt_{0}".format(name)])
                return
...
def is_fuse_exec(cmd):
    ...
    """
    cmd_path = hubblestack.utils.path.which(cmd)

    # No point in running ldd on a command that doesn't exist
    if not cmd_path:
        return False
    elif not hubblestack.utils.path.which("ldd"):
        raise CommandNotFoundError("ldd")

    out = __mods__["cmd.run"]("ldd {0}".format(cmd_path), python_shell=False)
...
def swaps():
    ...
    ret = {}
    if __grains__["kernel"] == "SunOS":
        for line in __mods__["cmd.run_stdout"]("swap -l").splitlines():
        ...
        with hubblestack.utils.files.fopen("/proc/swaps") as fp_:
            for line in fp_:
                line = hubblestack.utils.stringutils.to_unicode(line)
        ...
        for line in __mods__["cmd.run_stdout"]("swapctl -kl").splitlines():
...
def swapon(name, priority=None):
    ...
    if __grains__["kernel"] == "SunOS":
        if __grains__["virtual"] != "zone":
            __mods__["cmd.run"]("swap -a {0}".format(name), python_shell=False)
        else:
            return False
    else:
        ...
        __mods__["cmd.run"](cmd, python_shell=False)
...
def swapoff(name):
    ...
    if name in on_:
        if __grains__["kernel"] == "SunOS":
            ...
            __mods__["cmd.run"]("swapoff {0}".format(name), python_shell=False)
        else:
            __mods__["cmd.run"]("swapctl -d {0}".format(name), python_shell=False)
        on_ = swaps()
        if name in on_:
            return False
        return True
...
def read_mount_cache(name):
    ...
    """
    cache = hubblestack.utils.mount.read_cache(__opts__)
    if cache:
...
def write_mount_cache(real_name, device, mkmnt, fstype, mount_opts):
    ...
    cache_write = hubblestack.utils.mount.write_cache(cache, __opts__)
    if cache_write:
        return True
    else:
        ...
        try:
            with hubblestack.utils.files.fopen(config, "wb") as ofile:
                ...
                for fsys_view in view_lines:
                    ...
                    ofile.writelines(hubblestack.utils.data.encode(mystrg))
        except (IOError, OSError):
            raise CommandExecutionError("File not writable {0}".format(config))
