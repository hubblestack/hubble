...
def __virtual__():
    ...
    '''
    return (bool(hubblestack.utils.path.which_bin(['dmidecode', 'smbios'])),
...
def _dmidecoder(args=None):
    ...
    Call DMIdecode
    '''
    dmidecoder = hubblestack.utils.path.which_bin(['dmidecode', 'smbios'])

    if not args:
        out = hubblestack.modules.cmdmod._run_quiet(dmidecoder)
    else:
        out = hubblestack.modules.cmdmod._run_quiet('{0} {1}'.format(dmidecoder, args))
