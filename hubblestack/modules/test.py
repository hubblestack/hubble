...
def ping():
    ...
    if not hubblestack.utils.platform.is_proxy():
...
def version():
    ...
    .. code-block:: bash
        ...
        salt '*' test.version
    '''
    return hubblestack.version.__version__
...
def arg_clean(*args, **kwargs):
    ...
    '''
    return dict(args=args, kwargs=hubblestack.utils.args.clean_kwargs(**kwargs))
...
def not_loaded():
    ...
    prov = providers()
    ret = set()
    for mod_dir in hubblestack.loader._module_dirs(__opts__, 'modules', 'module'):
...
def random_hash(size=9999999999, hash_type=None):
    ...
    return hubblestack.utils.hashutils.random_hash(size=size, hash_type=hash_type)
...
def raise_exception(name, *args, **kwargs):
    '''
    Raise an exception. Built-in exceptions and those in ``hubblestack.exceptions``
    ...
    .. code-block:: bash
        ...
        salt '*' test.raise_exception hubblestack.exceptions.CommandExecutionError "Something went wrong"
    '''
    def _is_exc(cls):
    ...
    try:
        if name.startswith('hubblestack.exceptions.'):
            exc = getattr(hubblestack.exceptions, name[16:])
        else:
            exc = getattr(builtins, name)
        if _is_exc(exc):
            raise exc(*args, **hubblestack.utils.args.clean_kwargs(**kwargs))
        else:
