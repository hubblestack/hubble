...
def __virtual__():
    ...
    if hubblestack.utils.systemd.booted(__context__):
        ...
        if os.path.isfile(debian_initctl):
            initctl_version = hubblestack.modules.cmdmod._run_quiet(debian_initctl + ' version')
...
def _iter_service_names():
    ...
    # is named net/apache'
    init_root = '/etc/init/'
    for root, dirnames, filenames in hubblestack.utils.path.os_walk(init_root):
...
def _upstart_is_disabled(name):
    ...
    for file_name in itertools.ifilter(os.path.isfile, files):
        with hubblestack.utils.files.fopen(file_name) as fp_:
            if re.search(r'^\s*manual',
                         hubblestack.utils.stringutils.to_unicode(fp_.read()),
                         re.MULTILINE):
                return True
    return False
