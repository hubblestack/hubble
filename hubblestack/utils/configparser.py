...
class GitConfigParser(RawConfigParser, object):  # pylint: disable=undefined-variable
    ...
    >>> with hubblestack.utils.files.fopen('/home/user/.git/config', 'w') as fh:
    ...     conf.write(fh)
    >>>
    '''
    DEFAULTSECT = 'DEFAULT'
    ...
    def _read(self, fp, fpname):
        ...
        while True:
            line = hubblestack.utils.stringutils.to_unicode(fp.readline())
            if not line:
                break
            lineno = lineno + 1
            ...
            # no section header in the file?
                ...
                raise MissingSectionHeaderError(  # pylint: disable=undefined-variable
                    hubblestack.utils.stringutils.to_str(fpname),
                    lineno,
                    hubblestack.utils.stringutils.to_str(line))
            # an option line?
            else:
        ...
        if not section or section == self.DEFAULTSECT:
            ...
            try:
                ...
                raise NoSectionError(  # pylint: disable=undefined-variable
                    hubblestack.utils.stringutils.to_str(section))
    ...
    def write(self, fp_):
        ...
        4. Drops support for continuation lines.
        '''
        convert = hubblestack.utils.stringutils.to_bytes \
            if 'b' in fp_.mode \
            else hubblestack.utils.stringutils.to_str
        if self._defaults:
            fp_.write(convert('[%s]\n' % self.DEFAULTSECT))
            for (key, value) in iter(self._defaults.items()):
                value = hubblestack.utils.stringutils.to_unicode(value).replace('\n', '\n\t')
                fp_.write(convert('%s = %s\n' % (key, value)))
        for section in self._sections:
            fp_.write(convert('[%s]\n' % section))
            for (key, value) in iter(self._sections[section].items()):
