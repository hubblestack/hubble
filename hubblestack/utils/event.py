...
import hubblestack.utils.cache
import hubblestack.utils.dicttrim
import hubblestack.utils.platform
import hubblestack.utils.stringutils
import hubblestack.transport.ipc
...
def get_event(
    ...
    if transport in ('zeromq', 'tcp'):
        ...
        import hubblestack.utils.raetevent
        return hubblestack.utils.raetevent.RAETEvent(node,
                                              sock_dir=sock_dir,
                                              listen=listen,
                                              opts=opts)
...
def tagify(suffix='', prefix='', base=SALT):
    ...
    for index, _ in enumerate(parts):
        try:
            parts[index] = hubblestack.utils.stringutils.to_str(parts[index])
        except TypeError:
...
class SaltEvent(object):
    ...
    def __init__(
        ...
        if hubblestack.utils.platform.is_windows() and 'ipc_mode' not in opts:
    ...
    def __load_cache_regex(cls):
        ...
        cls.cache_regex = hubblestack.utils.cache.CacheRegex(prepend='^')
    ...
    def __load_uri(self, sock_dir, node):
        ...
        if node == 'master':
            ...
            if self.opts['ipc_mode'] == 'tcp':
                ...
                id_hash = hash_type(hubblestack.utils.stringutils.to_bytes(self.opts['id'])).hexdigest()[:10]
                puburi = os.path.join(
                    sock_dir,
        ...
        if self._run_io_loop_sync:
            with hubblestack.utils.asynchronous.current_ioloop(self.io_loop):
                if self.subscriber is None:
                    self.subscriber = hubblestack.transport.ipc.IPCMessageSubscriber(
                        self.puburi,
        ...
        if self._run_io_loop_sync:
            with hubblestack.utils.asynchronous.current_ioloop(self.io_loop):
                if self.pusher is None:
                    self.pusher = hubblestack.transport.ipc.IPCMessageClient(
                        self.pulluri,
                        io_loop=self.io_loop
                    )
                try:
    ...
    def unpack(cls, raw, serial=None):
        ...
        mtag, sep, mdata = raw.partition(hubblestack.utils.stringutils.to_bytes(TAGEND))  # split tag from data
        mtag = hubblestack.utils.stringutils.to_str(mtag)
    ...
    def fire_event(self, data, tag, timeout=1000):
        ...
        serialized_data = hubblestack.utils.dicttrim.trim_dict(
            dump_data,
        ...
        event = b''.join([
            hubblestack.utils.stringutils.to_bytes(tag),
            hubblestack.utils.stringutils.to_bytes(tagend),
            serialized_data])
        msg = hubblestack.utils.stringutils.to_bytes(event, 'utf-8')
        if self._run_io_loop_sync:
            with hubblestack.utils.asynchronous.current_ioloop(self.io_loop):
                try:
    ...
    def _fire_ret_load_specific_fun(self, load, fun_index=0):
        ...
        try:
            for tag, data in iter(ret.items()):
