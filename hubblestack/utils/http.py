...
def query(url,
    ...
    if cookie_jar is None:
        cookie_jar = os.path.join(opts.get('cachedir', hubblestack.syspaths.CACHE_DIR), 'cookies.txt')
    if session_cookie_jar is None:
        session_cookie_jar = os.path.join(opts.get('cachedir', hubblestack.syspaths.CACHE_DIR), 'cookies.session.p')
    ...
    if persist_session is True and HAS_MSGPACK:
        ...
        if os.path.isfile(session_cookie_jar):
            with hubblestack.utils.files.fopen(session_cookie_jar, 'rb') as fh_:
                session_cookies = msgpack.load(fh_)
            if isinstance(session_cookies, dict):
                header_dict.update(session_cookies)
        else:
            with hubblestack.utils.files.fopen(session_cookie_jar, 'wb') as fh_:
                msgpack.dump('', fh_)
    ...
    if backend == 'requests':
        ...
        if proxy_username:
            ...
            proxy_username = hubblestack.utils.stringutils.to_str(proxy_username)
        proxy_password = opts.get('proxy_password', None)
        if proxy_password:
            # tornado requires a str, cannot be unicode str in py2
            proxy_password = hubblestack.utils.stringutils.to_str(proxy_password)
    ...
    if decode is True:
        ...
        if decode_type == 'json':
            ...
            ret['dict'] = hubblestack.utils.data.decode(hubblestack.utils.yaml.safe_load(result_text))
        else:
            text = True
...
def get_ca_bundle(opts=None):
    ...
    file_roots = opts.get('file_roots', {'base': [hubblestack.syspaths.SRV_ROOT_DIR]})
    ...
    if hubblestack.utils.platform.is_windows() and HAS_CERTIFI:
...
def _render(template, render, renderer, template_dict, opts):
    ...
    if render:
        ...
        rend = hubblestack.loader.render(opts, {})
        ...
        if str(ret).startswith('#!') and not str(ret).startswith('#!/'):
            ret = str(ret).split('\n', 1)[1]
        return ret
    with hubblestack.utils.files.fopen(template, 'r') as fh_:
        return fh_.read()
