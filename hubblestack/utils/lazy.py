# -*- coding: utf-8 -*-
'''
Lazily-evaluated data structures, primarily used by Hubble's loader
'''
...
def verify_fun(lazy_obj, fun):
    ...
    '''
    if not fun:
        raise exceptions.HubbleInvocationError(
            'Must specify a function to run!\n'
            'ex: manage.up'
        )
    if fun not in lazy_obj:
        # If the requested function isn't available, lets say why
        raise exceptions.CommandExecutionError(lazy_obj.missing_fun_string(fun))
