...
import hubblestack.utils.kinds as kinds
from hubblestack import transport
from hubblestack import syspaths
...
class RAETEvent(object):
    '''
    The base class used to manage events
    '''
    ...
    def fire_ret_load(self, load):
        ...
        if load.get('retcode') and load.get('fun'):
            ...
            if load['fun'] in hubblestack.utils.event.SUB_EVENT:
                try:
                    for tag, data in iter(load.get('return', {}).items()):
                        ...
                        if data.get('result') is False:
                            ...
                            self.fire_event(
                                data,
                                hubblestack.utils.event.tagify([load['jid'],
                                        'sub',
                                        load['id'],
                                        'error',
                                        load['fun']],
