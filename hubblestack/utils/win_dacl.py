...
Windows DACL
============
This utility contains objects and functions for setting permissions to
...
def dacl(obj_name=None, obj_type="file"):
    ...
    class Dacl(flags(False)):
        ...
        def add_ace(self, principal, access_mode, permissions, applies_to):
            ...
            if self.dacl is None:
                raise HubbleInvocationError("You must load the DACL before adding an ACE")
...
def get_sid_string(principal):
    ...
    .. code-block:: python
        ...
        # Get a PySID object
        py_sid = hubblestack.utils.win_dacl.get_sid('jsnuffy')

        # Get the string version of the SID
        hubblestack.utils.win_dacl.get_sid_string(py_sid)
    """
    # If None is passed, use the Universal Well-known SID "Null SID"
    if principal is None:
        principal = "NULL SID"
...
def get_owner(obj_name, obj_type="file"):
    ...
    .. code-block:: python
        ...
        hubblestack.utils.win_dacl.get_owner('c:\\file')
    """
    ...
    try:
        ...
        raise HubbleInvocationError('Invalid "obj_type" passed: {0}'.format(obj_type))
...
def get_primary_group(obj_name, obj_type="file"):
    ...
    .. code-block:: python
        ...
        hubblestack.utils.win_dacl.get_primary_group('c:\\file')
    """
    # Not all filesystems mountable within windows have SecurityDescriptors.
    # For instance, some mounted SAMBA shares, or VirtualBox shared folders. If
...
def has_permission(
    ...
    if not chk_flag:
        raise HubbleInvocationError('Invalid "permission" passed: {0}'.format(permission))
                        ...
                        for flag in flags().ace_prop[obj_type]:
                            if (
                                ...
                                for flag1 in flags().ace_prop[obj_type]:
                                    if (
                                        hubblestack.utils.win_dacl.flags().ace_prop[obj_type][
                                            flag1
                                        ]
                                        == at_flag
                                    ):
