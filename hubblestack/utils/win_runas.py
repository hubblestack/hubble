...
try:
    ...
    import win32profile
    import msvcrt
    import hubblestack.platform.win
    import pywintypes
    HAS_WIN32 = True
except ImportError:
    HAS_WIN32 = False
...
def runas_unpriv(cmd, username, password, cwd=None):
    ...
    # inheritable.
    c2pread, c2pwrite = hubblestack.platform.win.CreatePipe(
        inherit_read=False, inherit_write=True,
    )
    errread, errwrite = hubblestack.platform.win.CreatePipe(
        inherit_read=False, inherit_write=True,
    )

    # Create inheritable copy of the stdin
    stdin = hubblestack.platform.win.kernel32.GetStdHandle(
            hubblestack.platform.win.STD_INPUT_HANDLE,
    )
    dupin = hubblestack.platform.win.DuplicateHandle(srchandle=stdin, inherit=True)
    ...
    try:
        ...
        process_info = hubblestack.platform.win.CreateProcessWithLogonW(
                username=username,
                domain=domain,
                password=password,
                logonflags=hubblestack.platform.win.LOGON_WITH_PROFILE,
                commandline=cmd,
                startupinfo=startup_info,
                currentdirectory=cwd)
        hubblestack.platform.win.kernel32.CloseHandle(process_info.hThread)
    finally:
        hubblestack.platform.win.kernel32.CloseHandle(dupin)
        hubblestack.platform.win.kernel32.CloseHandle(c2pwrite)
        hubblestack.platform.win.kernel32.CloseHandle(errwrite)
    ...
    # Get Return Code
    if hubblestack.platform.win.kernel32.WaitForSingleObject(process_info.hProcess, win32event.INFINITE) == \
            win32con.WAIT_OBJECT_0:
        exitcode = hubblestack.platform.win.wintypes.DWORD()
        hubblestack.platform.win.kernel32.GetExitCodeProcess(process_info.hProcess,
                                    ctypes.byref(exitcode))
        ret['retcode'] = exitcode.value

    # Close handle to process
    hubblestack.platform.win.kernel32.CloseHandle(process_info.hProcess)
