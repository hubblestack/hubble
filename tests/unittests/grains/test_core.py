...
from tests.support.mock import Mock, MagicMock, patch, mock_open, NO_MOCK, NO_MOCK_REASON
...
class CoreGrainsTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_parse_cpe_name_v23(self):
        ...
        for cpe, cpe_ret in [
            ...
            (
                ...
                {"phase": "beta", "version": "3.0", "product": "light_saber", "vendor": "dark_empire", "part": None},
            ),
        ]:
            ret = core._parse_cpe_name(cpe)
            for key in cpe_ret:
    ...
    def test_gnu_slash_linux_in_os_name(self):
        ...
        with patch.object(hubblestack.utils.platform, "is_proxy", MagicMock(return_value=False)), patch.object(
            core, "_linux_bin_exists", MagicMock(return_value=False)
        ), patch.object(os.path, "exists", path_exists_mock), patch(
            ...
            core.__mods__, {"cmd.run": cmd_run_mock}
        ):
    ...
    def test_suse_os_from_cpe_data(self):
        ...
        with patch.object(hubblestack.utils.platform, "is_proxy", MagicMock(return_value=False)), patch.object(
            ...
            core.__mods__, {"cmd.run": osarch_mock}
        ):
            os_grains = core.os_data()
    ...
    def _run_os_grains_tests(self, os_release_filename, os_release_map, expectation):
        ...
        if os_release_filename:
            os_release_data = core._parse_os_release(os.path.join(OS_RELEASE_DIR, os_release_filename))
        else:
        ...
        with patch.object(hubblestack.utils.platform, "is_proxy", MagicMock(return_value=False)), patch.object(
            core, "_linux_bin_exists", MagicMock(return_value=False)
        ), patch.object(os.path, "exists", path_isfile_mock), patch(
            "{0}.__import__".format(built_in), side_effect=_import_mock
        ), patch.object(
            os.path, "isfile", path_isfile_mock
        ), patch.object(
            ...
            "hubblestack.utils.files.fopen", mock_open(read_data=file_contents)
        ), patch.object(
            core, "linux_distribution", distro_mock
        ), patch.object(
            core, "_linux_gpu_data", empty_mock
    ...
    def test_bsd_memdata(self):
        ...
        with patch("platform.uname", MagicMock(return_value=mock_freebsd_uname)):
            with patch.object(hubblestack.utils.platform, "is_linux", MagicMock(return_value=False)):
                with patch.object(hubblestack.utils.platform, "is_freebsd", MagicMock(return_value=True)):
                    # Skip the first if statement
                                    ...
                                    with patch.object(core, "_hw_data", empty_mock):
                                        with patch.object(core, "_virtual", empty_mock):
                                            with patch.object(core, "_ps", empty_mock):
                                                # Mock the osarch
                                                with patch.dict(core.__mods__, {"cmd.run": cmd_run_mock}):
                                                    os_grains = core.os_data()
    ...
    def test_docker_virtual(self):
        ...
        with patch.object(os.path, "isdir", MagicMock(return_value=False)):
            with patch.object(
                os.path, "isfile", MagicMock(side_effect=lambda x: True if x == "/proc/1/cgroup" else False)
            ):
                for cgroup_substr in (":/system.slice/docker", ":/docker/", ":/docker-ce/"):
                    ...
                    with patch("hubblestack.utils.files.fopen", mock_open(read_data=cgroup_data)):
                        with patch.dict(core.__mods__, {"cmd.run_all": MagicMock()}):
                            self.assertEqual(core._virtual({"kernel": "Linux"}).get("virtual_subtype"), "Docker")
    ...
    def test_fqdn_return(self):
        ...
        self._run_fqdn_tests(net_ip4_mock, net_ip6_mock, ip4_empty=False, ip6_empty=False)
    ...
    def _run_fqdn_tests(self, net_ip4_mock, net_ip6_mock, ip6_empty=True, ip4_empty=True):
        ...
        ip6_mock = [(10, 1, 6, "", (IP6_ADD1, 0, 0, 0)), (10, 3, 0, "", (IP6_ADD2, 0, 0, 0))]

        with patch.dict(core.__opts__, {"ipv6": False}):
            with patch.object(hubblestack.utils.network, "ip_addrs", MagicMock(return_value=net_ip4_mock)):
                with patch.object(hubblestack.utils.network, "ip_addrs6", MagicMock(return_value=net_ip6_mock)):
    ...
    def test_dns_return(self):
        ...
        resolv_mock = {
            "domain": "",
            "sortlist": [],
            "nameservers": [ipaddress.IPv4Address(IP4_ADD1), ipaddress.IPv6Address(IP6_ADD1), IP6_ADD_SCOPE],
    ...
    def test_core_virtual(self):
        ...
        """
        virt = "kvm"
        with patch.object(hubblestack.utils.platform, "is_windows", MagicMock(return_value=False)):
            with patch.object(hubblestack.utils.path, "which", MagicMock(return_value=True)):
                with patch.dict(
                    core.__mods__,
                    {"cmd.run_all": MagicMock(return_value={"pid": 78, "retcode": 0, "stderr": "", "stdout": virt})},
                ):
                    osdata = {
                        "kernel": "test",
                    }
        ...
        with patch.object(hubblestack.utils.platform, "is_windows", MagicMock(return_value=False)):
            with patch.object(hubblestack.utils.path, "which", MagicMock(return_value=True)):
                with patch.dict(
                    ...
                    with patch("hubblestack.utils.files.fopen", mock_open(read_data="嗨")):
                        osdata = {
                            "kernel": "Linux",
                        }
                        ret = core._virtual(osdata)
