...
class MockTimedProc(object):
    '''
    Class used as a stand-in for hubblestack.utils.timed_subprocess.TimedProc
    '''
...
class CMDMODTestCase(TestCase, LoaderModuleMockMixin):
    '''
    Unit tests for the hubblestack.modules.cmdmod module
    '''
    ...
    def test_run_shell_is_not_file(self):
        ...
        '''
        with patch('hubblestack.modules.cmdmod._is_valid_shell', MagicMock(return_value=True)):
            with patch('hubblestack.utils.platform.is_windows', MagicMock(return_value=False)):
    ...
    def test_run_runas_with_windows(self):
        ...
        '''
        with patch('hubblestack.modules.cmdmod._is_valid_shell', MagicMock(return_value=True)):
            with patch('hubblestack.utils.platform.is_windows', MagicMock(return_value=True)):
    ...
    def test_run_user_not_available(self):
        ...
        with patch('hubblestack.modules.cmdmod._is_valid_shell', mock_true), \
    ...
    def test_run_no_vt_os_error(self):
        ...
        with patch('hubblestack.modules.cmdmod._is_valid_shell', MagicMock(return_value=True)):
            with patch('hubblestack.utils.platform.is_windows', MagicMock(return_value=False)):
                with patch('os.path.isfile', MagicMock(return_value=True)):
                    with patch('os.access', MagicMock(return_value=True)):
                        with patch('hubblestack.utils.timed_subprocess.TimedProc', MagicMock(side_effect=OSError)):
    ...
    def test_run_no_vt_io_error(self):
        ...
        with patch('hubblestack.modules.cmdmod._is_valid_shell', MagicMock(return_value=True)):
            with patch('hubblestack.utils.platform.is_windows', MagicMock(return_value=False)):
                with patch('os.path.isfile', MagicMock(return_value=True)):
                    with patch('os.access', MagicMock(return_value=True)):
                        with patch('hubblestack.utils.timed_subprocess.TimedProc', MagicMock(side_effect=IOError)):
                            self.assertRaises(CommandExecutionError, cmdmod._run, 'foo')

    @skipIf(hubblestack.utils.platform.is_windows(), 'Do not run on Windows')
        ...
        with patch('pwd.getpwnam') as getpwnam_mock:
            with patch('subprocess.Popen') as popen_mock:
                ...
                with patch.dict(cmdmod.__grains__, {'os': 'Darwin', 'os_family': 'Solaris'}):
                    ...
                    if not hubblestack.utils.platform.is_darwin():
