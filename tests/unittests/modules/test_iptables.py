...
# Import Hubble Libs
import hubblestack.modules.iptables as iptables
...
class IptablesTestCase(TestCase, LoaderModuleMockMixin):
    """
    Test cases for hubblestack.modules.iptables
    """
    ...
    def test_version(self):
        ...
        with patch.dict(iptables.__mods__, {"cmd.run": mock}):
    ...
    def test_save(self):
        ...
        """
        with patch("hubblestack.modules.iptables._conf", MagicMock(return_value=False)), patch(
    ...
    def test_check(self):
        ...
        with patch.object(iptables, "_has_option", mock_not):
            with patch.object(uuid, "getnode", MagicMock(return_value=mock_uuid)):
                with patch.dict(iptables.__mods__, {"cmd.run": mock_cmd}):
                    self.assertTrue(
                        iptables.check(
                            table="filter",
                            chain=mock_chain,
        ...
        with patch.object(iptables, "_has_option", mock_not):
            with patch.object(uuid, "getnode", MagicMock(return_value=mock_uuid)):
                with patch.dict(
                    iptables.__mods__, {"cmd.run": MagicMock(return_value="")}
                ):
                    self.assertFalse(
                        iptables.check(
                            table="filter",
