...
import hubblestack.modules.mount as mount
...
class MountTestCase(TestCase, LoaderModuleMockMixin):
    """
    Test cases for hubblestack.modules.mount
    """
    ...
    def test_fstab(self):
        ...
        with patch.dict(mount.__grains__, {"kernel": ""}), patch.object(
            os.path, "isfile", mock
        ), patch("hubblestack.utils.files.fopen", mock_open(read_data=file_data)):
            fstab = mount.fstab()
            assert fstab == {
                "B": {
                    "device": "A",
    ...
    def test_set_fstab(self):
        ...
        with patch.object(os.path, "isfile", mock):
            with patch.object(hubblestack.utils.files, "fopen", mock_read):
                self.assertRaises(CommandExecutionError, mount.set_fstab, "A", "B", "C")

        mock = MagicMock(return_value=True)
        with patch.object(os.path, "isfile", mock):
            with patch("hubblestack.utils.files.fopen", mock_open(read_data=MOCK_SHELL_FILE)):
    ...
    def test_set_automaster(self):
        ...
        with patch.object(os.path, "isfile", mock):
            with patch(
                "hubblestack.utils.files.fopen", mock_open(read_data="/..A -fstype=C,D C:B")
            ):
        ...
        with patch.object(os.path, "isfile", mock):
            with patch(
                "hubblestack.utils.files.fopen", mock_open(read_data="/..A -fstype=XX C:B")
            ):
                self.assertEqual(
    ...
    def test_umount(self):
        ...
        mock = MagicMock()
        with patch.dict(mount.__mods__, {"guestfs.umount": mock}):
    ...
    def test_is_fuse_exec(self):
        ...
        """
        with patch.object(hubblestack.utils.path, "which", return_value=None):
        ...
        with patch.object(hubblestack.utils.path, "which", which_mock):
            with patch.dict(mount.__mods__, {"cmd.run": _ldd_side_effect}):
    ...
    def test_swaps(self):
        ...
        with patch.dict(mount.__grains__, {"os": "", "kernel": ""}):
            with patch("hubblestack.utils.files.fopen", mock_open(read_data=file_data)):
                swaps = mount.swaps()
                assert swaps == {
                    "/dev/sda1": {
                        "priority": "-1",
        ...
        ), patch.dict(mount.__mods__, {"cmd.run_stdout": mock}):
            swaps = mount.swaps()
            assert swaps == {
                "/dev/sda1": {
                    "priority": "-1",
        ...
        with patch.dict(mount.__grains__, {"os": "AIX", "kernel": "AIX"}), patch.dict(
            mount.__mods__, {"cmd.run_stdout": mock}
        ):
            swaps = mount.swaps()
            assert swaps == {
                "/dev/hd6": {
    ...
    def test_swapon(self):
        ...
        with patch.dict(mount.__grains__, {"kernel": ""}):
            with patch.object(mount, "swaps", mock):
                mock = MagicMock(return_value=None)
                with patch.dict(mount.__mods__, {"cmd.run": mock}):
