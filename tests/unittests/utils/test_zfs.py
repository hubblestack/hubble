...
def _from_auto(name, value, source='auto'):
    ...
    '''
    with patch.object(hubblestack.utils.zfs, 'property_data_zpool', MagicMock(return_value=pmap_zpool)), \
         patch.object(hubblestack.utils.zfs, 'property_data_zfs', MagicMock(return_value=pmap_zfs)):
        return hubblestack.utils.zfs.from_auto(name, value, source)
...
def _from_auto_dict(values, source='auto'):
    ...
    with patch.object(hubblestack.utils.zfs, 'property_data_zpool', MagicMock(return_value=pmap_zpool)), \
         patch.object(hubblestack.utils.zfs, 'property_data_zfs', MagicMock(return_value=pmap_zfs)):
        return hubblestack.utils.zfs.from_auto_dict(values, source)
...
def _to_auto(name, value, source='auto', convert_to_human=True):
    ...
    with patch.object(hubblestack.utils.zfs, 'property_data_zpool', MagicMock(return_value=pmap_zpool)), \
         patch.object(hubblestack.utils.zfs, 'property_data_zfs', MagicMock(return_value=pmap_zfs)):
        return hubblestack.utils.zfs.to_auto(name, value, source, convert_to_human)
...
def _to_auto_dict(values, source='auto', convert_to_human=True):
    ...
    with patch.object(hubblestack.utils.zfs, 'property_data_zpool', MagicMock(return_value=pmap_zpool)), \
         patch.object(hubblestack.utils.zfs, 'property_data_zfs', MagicMock(return_value=pmap_zfs)):
        return hubblestack.utils.zfs.to_auto_dict(values, source, convert_to_human)
...
utils_patch = {
    ...
    #       hubblestack.utils.zfs but we cannot patch those using
...
class ZfsUtilsTestCase(TestCase):
    '''
    This class contains a set of functions that test hubblestack.utils.zfs utils
    '''
    ...
    def test_is_supported(self):
        ...
        for value in [False, True]:
            with patch('hubblestack.utils.path.which',
                       MagicMock(return_value=value)):
                with patch('hubblestack.utils.platform.is_linux',
                                  MagicMock(return_value=value)):
